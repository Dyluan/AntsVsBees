...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container:
            print(vars(self))
            #if 'ant' not in vars(self):
            if vars(self)['ant'] is None:
                if other.container is False:
                    return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place
        cp = 0
        while self.place.entrance is not None:
            #print(self.place.name)
            if self.place.bees and self.min_range < cp < self.max_range:
                temp = self.place
                self.place = current
                return random_or_none(temp.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <=0:
            for elem in self.place.bees:
                if self.place.exit == elem.place.exit:
                    self.place.remove_insect(elem)
